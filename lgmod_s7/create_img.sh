...
if [ -n "$S6" ]; then
	LGMOD_PLATFORM=S6
	LGMOD_VERSION="1.7.14"
	LGMOD_VERSION_EPK="37501"
	LGMOD_VERSION_ROOTFS="10714"
	size=1572864
...
if [ -n "$S6" ]; then
	...
	for I in A B C D Z; do # A=common S7; B=FW 4.01.xx 4.60.xx; C=US S7 model; D=LV340N-ZG; Z=without lginit binary
