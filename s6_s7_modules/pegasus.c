...
static const char driver_name[] = "pegasuslgmod";
...
static void set_ethernet_addr(pegasus_t * pegasus)
{
	...
	/* In pegasus II eeprom ethernet addr is copied from eeprom to register at HW reset automatically */
	/* So rely on this and read directly from registers */
	if (pegasus->features & PEGASUS_II) {
		ret = get_registers(pegasus, EthID, sizeof(node_id), node_id);
		/* But seems sometimes HW reset fails to copy and,or there is a timing problem */
		if ((node_id[0]+node_id[1]+node_id[2]+node_id[3]+node_id[4]+node_id[5]) == 0) {
			printk(KERN_WARNING "pegasus registers for ethernet adress not set reading eeprom\n");
			get_node_id(pegasus, node_id);
			set_registers(pegasus, EthID, sizeof (node_id), node_id);
		}
	/* In pegasus have to copy ethernet addr from eeprom to registers */
	} else {
		get_node_id(pegasus, node_id);
		set_registers(pegasus, EthID, sizeof (node_id), node_id);
	}
